...
import { connectAndReturnConn } from './index';

async function useConnection() {
  const conn = await connectAndReturnConn();

  if (conn) {
    mainInterface(conn);
    console.log('Connection is established:', conn);
  } else {
    console.log('No connection');
    // i2c display the connection failed
  }
}

// Call the function to use the connection
useConnection();

function mainInterface(conn: SoundcraftUI) {
